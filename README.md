...
Not versioned: the toolchain (`veaf-tools*.exe`, `published/`), the built `.miz`, and the
build reports. See `.gitignore`.
...
The output is `VEAF_Foothold_Caucasus_ICAO_URSS_<date>.miz`.

> **The file name matters.** `_ICAO_URSS` is read by the RealWeather extension of DCSServerBot,
> which fetches Sochi-Adler's live METAR at mission start. Renaming the mission without that
> marker silently disables real weather. See the
> [mission.yaml reference](https://veaf.github.io/documentation/) for the details.

## Updating to a new Lekaa release

Download the map's archive from [Lekaa's releases](https://github.com/leka1986/Lekas-Foothold/releases),
...
The `last-v5-version` tag marks the last state of this mission before the v6 migration — the
era when scripts were loaded from the server through a hand-written `VEAF_common.lua`.
